...
export default function ActivityCard({ 
  ...
  const renderLocation = () => {
    ...
    const handleCopyToGoogleMaps = async (e: React.MouseEvent) => {
      e.stopPropagation();
      try {
    ...
    return (
      <div className="mb-2.5 flex items-center gap-2 flex-wrap">
        ...
        <button
          onClick={handleCopyToGoogleMaps}
          className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-all duration-200"
          title="Copy to Google Maps"
        >
          {locationCopied ? (
            <>
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
